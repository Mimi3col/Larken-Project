
 __________________________________________
  __    __     _                           
 / / /\ \ \___| | ___ ___  _ __ ___   ___  
 \ \/  \/ / _ \ |/ __/ _ \| '_ ` _ \ / _ \ 
  \  /\  /  __/ | (_| (_) | | | | | |  __/ 
   \/  \/ \___|_|\___\___/|_| |_| |_|\___| 
 __________________________________________                                          

 Hello, Welcome to the game. To start off you need
 to know a couple commands....
  ----
 | cd | Allows you to move from Folder to Folder 
  ----  "Directory to Directory" 
       ------------------------
 EX :  | cd Start-Here (enter) |
       ------------------------
  ----
 | ls | Allows you to list everything in that 
  ----  Directory 
      ------------------------
 EX : | ls Start-Here (enter) | 
      ------------------------
  -----
 | cat | which will show you what is inside a file 
  ----- 
       ------------------
 EX : | cat Help (enter) | 
       ------------------
  ----
 | ./ | will run a program, if it has a .sh at the end 
  ----  it can be run.
        -----------------------
  EX : | ./Beginning.sh (enter) |
        -----------------------

   All statments need to be followed by an enter for
   the console to do the command

 Thank you for joining us on this mystery to find...
 _____ _              ___                      _      
/__   \ |__   ___    / __\___  _ __  ___  ___ | | ___ 
  / /\/ '_ \ / _ \  / /  / _ \| '_ \/ __|/ _ \| |/ _ \
 / /  | | | |  __/ / /__| (_) | | | \__ \ (_) | |  __/
 \/   |_| |_|\___| \____/\___/|_| |_|___/\___/|_|\___|
 -----------------------------------------------------                                                     
                     _ _ _                            
                /\ /(_) | | ___ _ __                  
               / //_/ | | |/ _ \ '__|                 
              / __ \| | | |  __/ |                    
              \/  \/|_|_|_|\___|_|                    
              -----------------------
=======

